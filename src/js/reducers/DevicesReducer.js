...
const authDevice = (state: State, device: TrezorDevice, deviceState: string): State => {
    const newState: State = [ ...state ];
    const index: number = state.findIndex(d => d.path === device.path && d.instance === device.instance);
    if (index >= 0) {
        // device could already have own state from DEVICE.CHANGE event
        // do not override it
        if (!newState[index].state) {
            const changedDevice: TrezorDevice = { 
                ...newState[index],
                state: deviceState
            };
            newState[index] = changedDevice;
        }
    }
    return newState;
}
...
const devicesFromStorage = (devices: Array<any>): State => {
